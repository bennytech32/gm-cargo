...
export default function Dashboard() {
  ...
  // --- KUPANGIA MZIGO (LOCAL STATE FOR NOW) ---
  const handleAssignLocal = (waybillId: string, field: string, value: string) => {
    ...
    // Tunatumia Local State kwa sasa maana Prisma Schema haijapokea hizi columns.
  };
  ...
  const handlePrintReceipt = (mzigo: any) => {
    ...
    const htmlContent = `
      <html>
        <head>
          <title>Risiti - ${mzigo.trackingNumber}</title>
          <style>
            body { font-family: monospace; padding: 20px; max-width: 350px; margin: 0 auto; color: #000; }
            h2 { text-align: center; margin: 0 0 10px 0; font-size: 24px; }
            .center { text-align: center; }
            .line { border-top: 1px dashed #000; margin: 15px 0; }
            .row { display: flex; justify-content: space-between; margin-bottom: 8px; font-size: 14px; }
            .bold { font-weight: bold; }
            .terms { font-size: 11px; margin-top: 20px; text-align: center; color: #333; }
          </style>
        </head>
        <body>
          <h2>GM CARGO</h2>
          <div class="center bold">Mzigo Traking Risiti</div>
          <div class="line"></div>
          <div class="row"><span>Tracking No:</span> <span class="bold">${mzigo.trackingNumber}</span></div>
          ...
          <div class="row"><span>Mkoa Unapoenda:</span> <span class="bold">${mzigo.destination}</span></div>
          <div class="line"></div>
          ...
          <div class="line"></div>
          <div class="center" style="font-size: 12px; margin-top: 10px;">Imehudumiwa na: ${runnerName}</div>
          <div class="terms">${receiptTerms.replace(/\n/g, '<br/>')}</div>
          <div class="center bold" style="margin-top: 15px;">Asante kwa kuchagua GM Cargo</div>
  ...
  // --- PRINT MANIFEST FUNCTION (NEW) ---
  const handlePrintManifest = (manifest: any) => {
    // Tafuta mizigo yote iliyopangiwa manifest hii
    ...
    const htmlContent = `
      <html>
        ...
        <body>
          <div class="header">
            <h1>GM CARGO - DISPATCH MANIFEST</h1>
            <p style="margin: 0; font-size: 18px;">Manifest No: <b>${manifest.id}</b></p>
          </div>
          <div class="details">
          ...
          <table>
            ...
            <tbody>
              ${manifestCargo.length > 0 ? manifestCargo.map((w: any, index: number) => {
                const agent = agents.find(a => a.id.toString() === w.agentId);
                return `
                <tr>
                  <td>${index + 1}</td>
                  <td><b>${w.trackingNumber}</b></td>
                  <td>${w.senderName}<br/>${w.senderPhone}</td>
                  <td>${w.receiverName}<br/>${w.receiverPhone}</td>
                  <td>${w.destination}</td>
                  <td>${agent ? agent.name : 'N/A'}</td>
                  <td></td>
                </tr>
              `}).join('') : '<tr><td colspan="7" style="text-align:center; padding: 20px;">Hakuna mizigo iliyopangiwa kwenye Manifest hii.</td></tr>'}
            </tbody>
          </table>
          <div class="footer">
            <div>
  ...
  // HELPER FUNCTION: Kutafuta Jina la Runner
  ...
  const handleRegisterCargo = async (e: any) => {
    ...
    try {
      const res = await fetch("/api/waybills", {
        ...
        body: JSON.stringify({ 
          ...cargoForm, registeredById: adminId, 
  ...
  const handleAddUser = async (e: any) => {
    ...
    try {
      ...
      if (res.ok) {
        alert("User Added!");
        setUserForm({ name: "", phone: "", password: "", role: "RUNNER" });
        fetchUsers();
      }
    } catch (error) { alert("Network Error"); } finally { setActionLoading(false); }
  };
  ...
  const handleAddAgent = (e: any) => {
    e.preventDefault();
    setAgents([{ id: Date.now(), ...agentForm }, ...agents]);
    setAgentForm({ name: "", region: "", phone: "", address: "" });
    alert("Agent Added (Local State)");
  };

  const handleAddManifest = (e: any) => {
    e.preventDefault();
    setManifests([{ id: `MNF-${Math.floor(Math.random() * 10000)}`, ...manifestForm, status: "PENDING" }, ...manifests]);
    setManifestForm({ driver: "", vehicle: "", route: "" });
    alert("Manifest Created (Local State)");
  };
  ...
  const filteredWaybills = waybills.filter((w: any) => 
    w.trackingNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
    w.senderPhone.includes(searchTerm) || w.receiverName.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col md:flex-row font-sans selection:bg-blue-200">
      ...
      {/* SIDEBAR */}
      <aside className="w-full md:w-72 bg-slate-900 text-white flex flex-col shadow-2xl z-20 sticky top-0 md:h-screen">
        ...
        <div className="flex-1 py-6 px-4 space-y-2 overflow-y-auto">
          <button onClick={() => setActiveTab("dashboard")} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all text-sm ${activeTab === 'dashboard' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
            <IconHome /> {t.sbDash}
          </button>
          <button onClick={() => setActiveTab("register")} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all text-sm ${activeTab === 'register' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
            <IconBox /> {t.sbReg}
          </button>
          <button onClick={() => setActiveTab("users")} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all text-sm ${activeTab === 'users' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
            <IconUsers /> {t.sbUsers}
          </button>
          <button onClick={() => setActiveTab("agents")} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all text-sm ${activeTab === 'agents' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
            <IconAgents /> {t.sbAgents}
          </button>
          <button onClick={() => setActiveTab("manifest")} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all text-sm ${activeTab === 'manifest' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
            <IconManifest /> {t.sbManifest}
          </button>
          <div className="my-4 border-t border-slate-800/50"></div>
          <button onClick={() => setActiveTab("settings")} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all text-sm ${activeTab === 'settings' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
            <IconSettings /> {t.sbSettings}
          </button>
        </div>
        ...
        <div className="p-4 border-t border-slate-800">
          ...
          <button onClick={handleLogout} className="w-full bg-red-500/10 text-red-500 border border-red-500/20 px-4 py-2.5 rounded-lg font-bold hover:bg-red-500 hover:text-white transition-colors text-sm">
            {t.logout}
          </button>
        </div>
      </aside>
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto h-screen bg-slate-50">
        ...
        {activeTab === "dashboard" && (
          <div className="animate-fade-in-up max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
              <div>
                <h1 className="text-3xl font-black text-slate-900">{t.dTitle}</h1>
                <p className="text-slate-500">{t.dSub}</p>
              </div>
              <input 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-md transition-shadow">
                <div className="absolute top-0 right-0 w-16 h-16 bg-blue-50 rounded-bl-full -mr-2 -mt-2 group-hover:scale-110 transition-transform"></div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 relative z-10">{t.dRev}</p>
                <p className="text-2xl font-black text-blue-600 relative z-10">TZS {totalRevenue.toLocaleString()}</p>
              </div>
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-md transition-shadow">
                <div className="absolute top-0 right-0 w-16 h-16 bg-green-50 rounded-bl-full -mr-2 -mt-2 group-hover:scale-110 transition-transform"></div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 relative z-10">{t.dVal}</p>
                <p className="text-2xl font-black text-green-600 relative z-10">TZS {totalCargoValue.toLocaleString()}</p>
              </div>
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-md transition-shadow">
                <div className="absolute top-0 right-0 w-16 h-16 bg-slate-50 rounded-bl-full -mr-2 -mt-2 group-hover:scale-110 transition-transform"></div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 relative z-10">{t.dTot}</p>
                <p className="text-2xl font-black text-slate-800 relative z-10">{waybills.length}</p>
              </div>
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-md transition-shadow">
                <div className="absolute top-0 right-0 w-16 h-16 bg-purple-50 rounded-bl-full -mr-2 -mt-2 group-hover:scale-110 transition-transform"></div>
                ...
                <p className="text-2xl font-black text-purple-600 relative z-10">{onlineRunnersCount}</p>
              </div>
            </div>
                ...
                <tbody className="divide-y divide-slate-50">
                  {filteredWaybills.map((mzigo: any) => (
                    <tr key={mzigo.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      {/* COLUMN 3: Destination & Assignments (NEW) */}
                      <td className="p-4">
                        ...
                        <div className="flex flex-col gap-1">
                          <select 
                            value={mzigo.manifestId || ""} 
                            onChange={(e) => handleAssignLocal(mzigo.id, 'manifestId', e.target.value)} 
                            className="text-[10px] font-bold text-slate-600 bg-white border border-slate-200 rounded p-1 outline-none focus:border-blue-400 w-full"
                          >
                          ...
                          <select 
                            value={mzigo.agentId || ""} 
                            onChange={(e) => handleAssignLocal(mzigo.id, 'agentId', e.target.value)} 
                            className="text-[10px] font-bold text-slate-600 bg-white border border-slate-200 rounded p-1 outline-none focus:border-blue-400 w-full"
                          >
                      ...
                      {/* COLUMN 5: Action & Print */}
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <select 
                            value={mzigo.status} onChange={(e) => updateStatus(mzigo.id, e.target.value)}
                            className="text-xs font-bold bg-white border border-slate-200 rounded-lg p-2 text-slate-700 outline-none cursor-pointer focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                          >
                          ...
                          <button 
                            onClick={() => handlePrintReceipt(mzigo)}
                            className="bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 p-2 rounded-lg transition-colors flex items-center justify-center"
                            title={t.btnPrint}
                          >
        ...
        {activeTab === "register" && (
          <div className="max-w-4xl mx-auto animate-fade-in-up">
            <h1 className="text-3xl font-black text-slate-900 mb-2">{t.rTitle}</h1>
            <p className="text-slate-500 mb-8">{t.rSub}</p>
            
            <form onSubmit={handleRegisterCargo} className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
        ...
        {activeTab === "users" && (
          <div className="animate-fade-in-up max-w-7xl mx-auto">
            <h1 className="text-3xl font-black text-slate-900 mb-2">{t.uTitle}</h1>
            <p className="text-slate-500 mb-8">{t.uSub}</p>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-1">
                <form onSubmit={handleAddUser} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 sticky top-6">
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {users.map((user: any) => (
                        <tr key={user.id} className="hover:bg-slate-50">
                          ...
                          <td className="p-4">
                            <span className={`px-3 py-1 rounded-full text-xs font-black ${user.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
        ...
        {activeTab === "agents" && (
          <div className="animate-fade-in-up max-w-7xl mx-auto">
            <h1 className="text-3xl font-black text-slate-900 mb-2">{t.aTitle}</h1>
            <p className="text-slate-500 mb-8">{t.aSub}</p>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-1">
                <form onSubmit={handleAddAgent} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 sticky top-6">
        ...
        {activeTab === "manifest" && (
          <div className="animate-fade-in-up max-w-7xl mx-auto">
            <h1 className="text-3xl font-black text-slate-900 mb-2">{t.mTitle}</h1>
            <p className="text-slate-500 mb-8">{t.mSub}</p>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-1">
                <form onSubmit={handleAddManifest} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 sticky top-6">
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {manifests.map((man: any) => (
                        <tr key={man.id} className="hover:bg-slate-50">
                          ...
                          {/* VIEW / PRINT MANIFEST BUTTON (NEW) */}
                          <td className="p-4">
                            <button 
        ...
        {activeTab === "settings" && (
          <div className="max-w-3xl mx-auto animate-fade-in-up">
            <h1 className="text-3xl font-black text-slate-900 mb-2">{t.sTitle}</h1>
            <p className="text-slate-500 mb-8">{t.sSub}</p>

            <form onSubmit={handleSaveSettings} className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
